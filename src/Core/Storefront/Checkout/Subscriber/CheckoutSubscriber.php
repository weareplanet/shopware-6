...
use WeArePlanetPayment\Core\Checkout\Service\PaymentMethodFilterService;
use WeArePlanetPayment\Core\Checkout\Service\PaymentIntegrationService;
...
class CheckoutSubscriber implements EventSubscriberInterface
{
    ...
    public function onPageLoaded($event): void
    {
        try {
            ...
            $transactionId = $salesChannelContext->getContext()->getExtension('weareplanet_transaction_id');
            if ($transactionId) {
                $paymentConfig = $this->paymentIntegrationService->getConfigForTransaction(
                    (int) $transactionId->getVars()['value'],
                    $salesChannelContext
                );
                $event->getPage()->addExtension('weArePlanetData', $paymentConfig);
            }
        } catch (\Exception $e) {
    ...
    public function onMailBeforeValidate(MailBeforeValidateEvent $event): void
    {
        ...
        if (!empty($order) && $order->getAmountTotal() > 0) {
            ...
            $isWeArePlanetEmailSettingEnabled = (bool)$this->settingsService->getSettings($order->getSalesChannelId())->isEmailEnabled();

            if (!$isWeArePlanetEmailSettingEnabled) {
                return;
            }
