...
class ActivePaymentRedirectSubscriber implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        // 'frontend.account.order.page'), a cookie weareplanet_deliberate was set and read here to 
        // prevent this function from wrongly redirecting the user to recreate-cart.
        if ($request->cookies->get('weareplanet_deliberate')) {
            $session->remove('weareplanetActivePaymentOrderId');
            return;
        }

        // If the user entered checkout, weareplanetActivePaymentOrderId should’ve been set.
        $orderId = (string) $session->get('weareplanetActivePaymentOrderId', '');
        if ($orderId === '') {
            return;
        }
