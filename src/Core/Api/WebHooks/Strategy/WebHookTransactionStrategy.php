...
class WebHookTransactionStrategy extends WebHookStrategyBase implements WebhookStrategyActionsInterface {
	...
	public function getOrderIdByTransaction(Transaction $transaction): string|null
	{
		...
		return $transaction->getMetaData()[TransactionPayload::WEAREPLANET_METADATA_ORDER_ID];
	}
	...
	/**
	 * Handles the processing of webhook callbacks related to WeArePlanet transactions.
	...
	private function processTransaction(WebHookRequest $request, Context $context): Response
	{
		...
		try {
			...
			$orderId = $transaction->getMetaData()[TransactionPayload::WEAREPLANET_METADATA_ORDER_ID] ?? null;
							...
							case TransactionState::AUTHORIZED:
                                if ($token instanceof Token) {
                                    ...
                                    $data = [
                                        ...
                                        'customFields' => [
                                            TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WEAREPLANET_TOKEN => $token->getId(),
                                        ],
                                    ];
