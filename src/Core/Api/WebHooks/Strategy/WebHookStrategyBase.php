...
namespace WeArePlanetPayment\Core\Api\WebHooks\Strategy;
 ...
 *
 * @package WeArePlanetPayment\Core\Api\WebHooks\Strategy
 */
abstract class WebHookStrategyBase implements WebHookStrategyInterface {
	...
	public $weareplanetTransactionSuccessStates = [
	 ...
	 * @param LoggerInterface $weareplanetPaymentLogger
	 * @see "$weareplanetPaymentLogger", please read the documentation "How to Autowire Logger Channels"
	 */
	public function __construct(
		...
		LoggerInterface $weareplanetPaymentLogger
	) {
		...
		$this->logger                            = $weareplanetPaymentLogger;
	}
	...
	protected function sendEmail(Transaction $transaction, Context $context, string $orderId): void
	{
		...
		if ($this->settings->isEmailEnabled()
			&& in_array($transaction->getState(), $this->weareplanetTransactionSuccessStates)) {
