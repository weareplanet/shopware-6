...
class TransactionService
{
     ...
     */
    public const FILTER_MEMO_EXTENSION = 'weareplanet_filter_memo';

    /**
     * Cache key prefix for the persisted pending transaction record.
     */
    private const PENDING_TRANSACTION_CACHE_PREFIX = 'plnn_pending_transaction_v2_';
    ...
    protected function addWeArePlanetTransactionId(
        ...
        if ($token) {
            $customFields[TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WEAREPLANET_TOKEN] = $token;
        }
    ...
    /**
     * Build a WeArePlanet address from Shopware customer address.
     *
     * @param \Shopware\Core\System\SalesChannel\SalesChannelContext $salesChannelContext
    ...
    private function extractShippingLineItem(
        ...
        $fallbackName = $translator ? $translator->trans('weareplanet.payload.shipping.name') : 'Shipping';
        ...
        $shippingLineItem->setAmountIncludingTax($roundedAmount)
            ->setName($this->fixLength($shippingName . ' ' . ($translator ? $translator->trans('weareplanet.payload.shipping.lineItem') : 'Shipping'), PayloadLimits::LINE_ITEM_NAME))
