...
class TransactionService
{
    ...
    private function getPendingTransactionCacheKey(SalesChannelContext $salesChannelContext): ?string
    {
        $customer = $salesChannelContext->getCustomer();
        if ($customer) {
            return 'plnn_pending_transaction_id_customer_' . $customer->getId();
        }
        return null;
    }
    ...
    private function extractShippingLineItem(
        ...
        $shippingLineItem->setAmountIncludingTax($roundedAmount)
            ->setName($this->fixLength($shippingName . ' ' . ($translator ? $translator->trans('weareplanet.payload.shipping.lineItem') : 'Shipping'), 150))
