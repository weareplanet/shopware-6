...
use WeArePlanetPayment\Core\Checkout\Cart\CustomCartPersister;
...
use WeArePlanetPayment\Core\Util\Payload\PayloadLimits;
 ...
 */
class WeArePlanetPaymentHandler extends AbstractPaymentHandler
{
    ...
    public function pay(
        ...
        try {
            ...
            if ($orderTransaction->getOrder()->getAmountTotal() > 0) {
                ...
                $request->getSession()->set('weareplanetActivePaymentOrderId', $orderId);
            }
    ...
    public function finalize(
        ...
        if ($request->hasSession()) {
            $request->getSession()->remove('weareplanetActivePaymentOrderId');
        }
        ...
        if ($orderTransaction->getOrder()->getAmountTotal() > 0) {
            ...
            if (in_array($weArePlanetTransaction->getState(), [TransactionState::FAILED, TransactionState::DECLINE, TransactionState::VOIDED,])) {
        ...
        if ($token) {
            ...
            $storedToken = $customFields[TransactionPayload::ORDER_TRANSACTION_CUSTOM_FIELDS_WEAREPLANET_TOKEN] ?? null;
