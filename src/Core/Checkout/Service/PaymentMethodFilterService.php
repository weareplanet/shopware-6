...
namespace WeArePlanetPayment\Core\Checkout\Service;
...
class PaymentMethodFilterService
{
     ...
     */
    private const POSSIBLE_METHODS_CACHE_PREFIX = 'plnn_possible_methods_';
    ...
    public function filterPaymentMethods(
        ...
        if (is_null($settings)) {
            return $this->removeWeArePlanetPaymentMethods($paymentMethodCollection, $salesChannelContext);
        }
     ...
     */
    private function removeWeArePlanetPaymentMethods(
        PaymentMethodCollection $paymentMethodCollection,
        SalesChannelContext $salesChannelContext
    ): PaymentMethodCollection {
        $paymentMethodIds = $this->paymentMethodUtil->getWeArePlanetPaymentMethodIds($salesChannelContext->getContext());
    ...
    private function buildFilteredCollection(
        ...
        foreach ($paymentMethodCollection as $method) {
            $isWeArePlanetPM = WeArePlanetPaymentHandler::class === $method->getHandlerIdentifier();
            ...
            if (isset($allowedWLConfigByPmId[$method->getId()])) {
                $method->addExtension('weareplanet_config', $allowedWLConfigByPmId[$method->getId()]);
                $collection->add($method);
            }
        }
