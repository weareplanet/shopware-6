...
class Migration1766067106TransactionEntity extends MigrationStep
{
    ...
    {
        $oldTableName = 'weareplanet_transaction';
        $tempTableName = 'weareplanet_transaction_tmp';
        $realTableName = 'weareplanet_transaction_data';
        $logger = new Logger('weareplanet_migration');
        $logger->pushHandler(new StreamHandler(dirname(__DIR__, 5) . '/var/log/weareplanet-migration.log'));
        $logger->info(
            'Migration start', [
        ...
        if ($this->tableExists($connection, $tempTableName)) {
            ...
            if ($this->tableExists($connection, $oldTableName) && !$this->isOldPluginTable($connection, $oldTableName)) {
                $logger->info('Old weareplanet_transaction table detected.');
                // If weareplanet_transaction already exists and does not belong to old plugin, 
                // it means that this is indeed a version update.
                $this->syncTransactionTable($connection, $oldTableName);
                $logger->info('Old weareplanet_transaction table sync finished.');
                $this->syncRefundTable($connection, $oldTableName);
                $logger->info('Old weareplanet_refund table sync finished.');
                $connection->executeStatement(
                    sprintf('RENAME TABLE `%s` TO `%s`', $oldTableName, $realTableName)
                );
                $logger->info('Old weareplanet_transaction table renaming completed.');
            }
            $logger->info('Possible plugin upgrade finished.');
            // If weareplanet_transaction exists and it does belong to old plugin, 
    ...
    private function syncTransactionTable(Connection $connection, string $table): void {
        ...
        $this->ensureIndexBySql($connection, $table, 'fk.pln_transaction.order_id', "KEY `fk.pln_transaction.order_id` (`order_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.pln_transaction.order_transaction_id', "KEY `fk.pln_transaction.order_transaction_id` (`order_transaction_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.pln_transaction.payment_method_id', "KEY `fk.pln_transaction.payment_method_id` (`payment_method_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.pln_transaction.sales_channel_id', "KEY `fk.pln_transaction.sales_channel_id` (`sales_channel_id`)");
        $this->ensureIndexBySql($connection, $table, 'fk.pln_transaction', "KEY `fk.pln_transaction` (`order_id`,`order_version_id`)");

        $this->ensureForeignKey(
            $connection,
            $table,
            'fk.pln_transaction_order_id',
        ...
        $this->ensureForeignKey(
            $connection,
            $table,
            'fk.pln_transaction_payment_method_id',
            ['payment_method_id'],
            'payment_method',
            ['id'],
            'RESTRICT',
        ...
        $this->ensureForeignKey(
            $connection,
            $table,
            'fk.pln_transaction_sales_channel_id',
            ['sales_channel_id'],
            'sales_channel',
            ['id'],
            'RESTRICT',
    ...
    private function syncRefundTable(Connection $connection, string $table): void {
        $refundTable = 'weareplanet_refund';
        $this->ensureIndexBySql($connection, $refundTable, 'fk.pln_refund.transaction_id', "KEY `fk.pln_refund.transaction_id` (`transaction_id`)");
        $this->ensureForeignKey(
            $connection,
            $refundTable,
            'fk.pln_refund.transaction_id',
            ['transaction_id'],
            $table,
            ['transaction_id'],
            'CASCADE',
