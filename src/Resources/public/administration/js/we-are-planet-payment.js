(()=>{var C=`{% block sw_order_detail_content_tabs_general %}
    {% parent %}
...
{% endblock %}
`;var{Component:ie,Context:y}=Shopware,oe=Shopware.Data.Criteria,se="handler_weareplanetpayment_weareplanetpaymenthandler";ie.override("sw-order-detail",{template:C,data(){return{isWeArePlanetPayment:!1}},computed:{isEditable(){return!this.isWeArePlanetPayment||this.$route.name!=="weareplanet.order.detail"},showTabs(){return!0}},watch:{orderId:{deep:!0,handler(){if(!this.orderId){this.setIsWeArePlanetPayment(null);return}let e=this.repositoryFactory.create("order"),t=new oe(1,1);t.addAssociation("transactions"),e.get(this.orderId,y.api,t).then(a=>{if(a.amountTotal<=0||a.transactions.length<=0||!a.transactions[0].paymentMethodId){this.setIsWeArePlanetPayment(null);return}let n=a.transactions[0].paymentMethodId;n!=null&&this.setIsWeArePlanetPayment(n)})},immediate:!0}},methods:{setIsWeArePlanetPayment(e){if(!e)return;this.repositoryFactory.create("payment_method").get(e,y.api).then(a=>{this.isWeArePlanetPayment=a.formattedHandlerIdentifier===se})}}});var A=`{% block weareplanet_order_action_completion %}
...
{% endblock %}
`;var{Component:ce,Mixin:de,Filter:ue,Utils:E}=Shopware;ce.register("weareplanet-order-action-completion",{template:A,inject:["WeArePlanetTransactionCompletionService"],mixins:[de.getByName("notification")],props:{transactionData:{type:Object,required:!0}},data(){return{isLoading:!0,isCompletion:!1}},computed:{dateFilter(){return ue.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1},completion(){this.isCompletion&&(this.isLoading=!0,this.WeArePlanetTransactionCompletionService.createTransactionCompletion(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id).then(()=>{this.createNotificationSuccess({title:this.$tc("weareplanet-order.captureAction.successTitle"),message:this.$tc("weareplanet-order.captureAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${E.createId()}`)})}).catch(e=>{try{this.createNotificationError({title:e.response.data.errors[0].title,message:e.response.data.errors[0].detail,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${E.createId()}`)})}}))}}});var S=`{% block weareplanet_order_action_refund %}
<sw-modal variant="small"
	...
	{% block weareplanet_order_action_refund_amount %}
		...
		<mt-number-field
			...
			number-type="int"
			 :label="$tc('weareplanet-order.refund.refundQuantity.label')">
		</mt-number-field>

		<div>
			{{ $tc('weareplanet-order.refundAction.maxAvailableItemsToRefund') }}:
...
{% endblock %}
`;var{Component:me,Mixin:he,Filter:ge,Utils:v}=Shopware;me.register("weareplanet-order-action-refund",{template:S,inject:["WeArePlanetRefundService"],mixins:[he.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{refundQuantity:0,isLoading:!0,currentLineItem:""}},computed:{dateFilter(){return ge.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.refundQuantity=1},refund(){this.isLoading=!0,this.WeArePlanetRefundService.createRefund(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundQuantity,this.$parent.$parent.currentLineItem).then(()=>{this.createNotificationSuccess({title:this.$tc("weareplanet-order.refundAction.successTitle"),message:this.$tc("weareplanet-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${v.createId()}`)})}).catch(e=>{try{var t=e?.response?.data?.errors?.[0]?.title??this.$tc("weareplanet-order.refundAction.refundCreateError.errorTitle"),a;switch(e.response.data){case"refundQuantityZero":a=this.$tc("weareplanet-order.refundAction.refundCreateError.messageRefundQuantityIsZero");break;case"refundExceedsQuantity":a=this.$tc("weareplanet-order.refundAction.refundCreateError.messageRefundQuantityExceedsAvailableBalance");break;case"methodDoesNotSupportRefund":a=this.$tc("weareplanet-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund");break;default:a=e.response.data.errors[0].detail}this.createNotificationError({title:t,message:a,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${v.createId()}`)})}})}}});var T=`{% block weareplanet_order_action_refund_partial %}
<sw-modal variant="small"
		  :title="$tc(\`weareplanet-order.modal.title.refund\`)"
		  @modal-close="$emit('modal-close')">

	{% block weareplanet_order_action_refund_amount_partial %}
		<mt-number-field
		...
		<div>
			{{ $tc('weareplanet-order.refundAction.maxAvailableAmountToRefund') }}:
			<b>{{ this.$parent.$parent.itemRefundableAmount }}</b>
		</div>
	{% endblock %}

	{% block weareplanet_order_action_refund_confirm_button_partial %}
	<template #modal-footer>
...
{% endblock %}
`;var{Component:be,Mixin:_e,Filter:we,Utils:P}=Shopware;be.register("weareplanet-order-action-refund-partial",{template:T,inject:["WeArePlanetRefundService"],mixins:[_e.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{isLoading:!0,currency:this.transactionData.transactions[0].currency,refundAmount:0}},computed:{dateFilter(){return we.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.currency=this.transactionData.transactions[0].currency,this.refundAmount||(this.refundAmount=this.$parent.$parent.itemRefundableAmount)},createPartialRefund(e){this.isLoading=!0,this.WeArePlanetRefundService.createPartialRefund(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundAmount,e).then(()=>{this.createNotificationSuccess({title:this.$tc("weareplanet-order.refundAction.successTitle"),message:this.$tc("weareplanet-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${P.createId()}`)})}).catch(t=>{try{var a=t?.response?.data?.errors?.[0]?.title??this.$tc("weareplanet-order.refundAction.refundCreateError.errorTitle"),n;t.response.data==="methodDoesNotSupportRefund"?n=this.$tc("weareplanet-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund"):n=t.response.data.errors[0].detail,this.createNotificationError({title:a,message:n,autoClose:!1})}catch{this.createNotificationError({title:t.title,message:t.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${P.createId()}`)})}})}},watch:{refundAmount(e){e!==null&&(this.refundAmount=Math.round(e*100)/100)}}});var D=`{% block weareplanet_order_action_refund_by_amount %}
<sw-modal variant="small"
		  :title="$tc(\`weareplanet-order.modal.title.refund\`)"
		  @modal-close="$emit('modal-close')">

	{% block weareplanet_order_action_refund_amount_by_amount %}
		<mt-number-field
	...
	{% block weareplanet_order_action_refund_confirm_button_by_amount %}
	<template #modal-footer>
...
{% endblock %}
`;var{Component:Ce,Mixin:ye,Filter:Ae,Utils:N}=Shopware;Ce.register("weareplanet-order-action-refund-by-amount",{template:D,inject:["WeArePlanetRefundService"],mixins:[ye.getByName("notification")],props:{transactionData:{type:Object,required:!0},orderId:{type:String,required:!0}},data(){return{isLoading:!0,currency:this.transactionData.transactions[0].currency,refundAmount:0,refundableAmount:0}},computed:{dateFilter(){return Ae.getByName("date")}},created(){this.createdComponent()},methods:{createdComponent(){this.isLoading=!1,this.currency=this.transactionData.transactions[0].currency,this.refundAmount=Number(this.transactionData.transactions[0].amountIncludingTax),this.refundableAmount=Number(this.transactionData.transactions[0].amountIncludingTax)},refundByAmount(){this.isLoading=!0,this.WeArePlanetRefundService.createRefundByAmount(this.transactionData.transactions[0].metaData.salesChannelId,this.transactionData.transactions[0].id,this.refundAmount).then(()=>{this.createNotificationSuccess({title:this.$tc("weareplanet-order.refundAction.successTitle"),message:this.$tc("weareplanet-order.refundAction.successMessage")}),this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${N.createId()}`)})}).catch(e=>{try{var t=e?.response?.data?.errors?.[0]?.title??this.$tc("weareplanet-order.refundAction.refundCreateError.errorTitle"),a;switch(e.response.data){case"refundAmountZero":a=this.$tc("weareplanet-order.refundAction.refundCreateError.messageRefundAmountIsZero");break;case"refundExceedsAmount":a=this.$tc("weareplanet-order.refundAction.refundCreateError.messageRefundAmountExceedsAvailableBalance");break;case"methodDoesNotSupportRefund":a=this.$tc("weareplanet-order.refundAction.refundCreateError.messagePaymentMethodDoesNotSupportRefund");break;default:a=e.response.data.errors[0].detail}this.createNotificationError({title:t,message:a,autoClose:!1})}catch{this.createNotificationError({title:e.title,message:e.message,autoClose:!1})}finally{this.isLoading=!1,this.$emit("modal-close"),this.$nextTick(()=>{this.$router.replace(`${this.$route.path}?hash=${N.createId()}`)})}})}}});var O=`{% block weareplanet_order_action_void %}
<sw-modal variant="small"
			...
			<sw-container>
							...
							<sw-inherit-wrapper
								...
								<template #content="props">
									<mt-number-field
											...
											:disabled="!acl.can('weareplanet.editor')"
				...
				<sw-container columns="1fr 1fr" gap="0px 30px">
					<mt-button
						...
						{{ $tc('weareplanet-settings.settingForm.credentials.button.label') }}
					</mt-button>
				</sw-container>
				{% endverbatim %}
...
{% endblock %}
`;var{Component:et,Mixin:tt}=Shopware;et.register("sw-weareplanet-credentials",{template:G,name:"WeArePlanetCredentials",inject:["acl"],mixins:[tt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{type:[String,null],required:!1,default:null},spaceIdFilled:{type:Boolean,required:!0},spaceIdErrorState:{required:!0},userIdFilled:{type:Boolean,required:!0},userIdErrorState:{required:!0},applicationKeyFilled:{type:Boolean,required:!0},applicationKeyErrorState:{required:!0},isLoading:{type:Boolean,required:!0},isTesting:{type:Boolean,required:!1}},data(){return{...l}},computed:{currentConfig(){return this.selectedSalesChannelId&&this.allConfigs[this.selectedSalesChannelId]?this.allConfigs[this.selectedSalesChannelId]:this.allConfigs.null||{}}},methods:{checkTextFieldInheritance(e){return!e||e.length<=0},checkNumberFieldInheritance(e){return e==null||e===""},checkBoolFieldInheritance(e){return typeof e!="boolean"},emitCheckApiConnectionEvent(){let e={spaceId:this.currentConfig[l.CONFIG_SPACE_ID],userId:this.currentConfig[l.CONFIG_USER_ID],applicationKey:this.currentConfig[l.CONFIG_APPLICATION_KEY]};this.$emit("check-api-connection-event",e)},getInheritedValue(e){return this.allConfigs.null?.[e]??null}}});var z=`{% block weareplanet_settings_content_card_channel_config_options %}
			...
			<sw-container>
				...
				{% block weareplanet_settings_content_card_channel_config_credentials_card_container_settings %}
					<div v-if="actualConfigData" class="weareplanet-settings-options-fields">
						...
						{% block weareplanet_settings_content_card_channel_config_credentials_card_container_settings_order_close_enabled %}
							<sw-inherit-wrapper
								...
								<template #content="props">
									<sw-switch-field
											...
											bordered
											:mapInheritance="props"
											:label="$tc('weareplanet-settings.settingForm.options.orderCloseEnabled.label')"
											:helpText="$tc('weareplanet-settings.settingForm.options.orderCloseEnabled.tooltipText')"
											:disabled="props.isInherited"
											:value="props.currentValue"
											@update:value="props.updateCurrentValue">
									</sw-switch-field>
						...
						{% block weareplanet_settings_content_card_channel_config_credentials_card_container_settings_product_custom_fields_allow_list %}
							<sw-inherit-wrapper
								...
								<template #content="props">
									<sw-multi-select
											...
											:mapInheritance="props"
											:label="$tc('weareplanet-settings.settingForm.options.productCustomFieldsAllowList.label')"
											:helpText="$tc('weareplanet-settings.settingForm.options.productCustomFieldsAllowList.tooltipText')"
											:disabled="props.isInherited"
											:value="props.currentValue || []"
											@update:value="props.updateCurrentValue">
									</sw-multi-select>
...
{% endblock %}
`;var{Component:nt,Mixin:rt,Context:it}=Shopware,g=Shopware.Data.Criteria;nt.register("sw-weareplanet-options",{template:z,name:"WeArePlanetOptions",inject:["repositoryFactory"],mixins:[rt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l,productCustomFields:[]}},created(){this.loadProductCustomFields()},computed:{customFieldRepository(){return this.repositoryFactory.create("custom_field")},productCustomFieldOptions(){let e=Shopware.Store&&Shopware.Store.get?Shopware.Store.get("session"):null,t=e&&e.currentLocale||"en-GB";return this.productCustomFields.map(a=>{let n=a.config&&a.config.label?a.config.label[t]||a.config.label["en-GB"]:null;return{value:a.name,label:n&&n!==a.name?`${n} (${a.name})`:a.name}})},integrationOptions(){return[{id:"payment_page",name:this.$tc("weareplanet-settings.settingForm.options.integration.options.payment_page")},{id:"iframe",name:this.$tc("weareplanet-settings.settingForm.options.integration.options.iframe")}]}},methods:{loadProductCustomFields(){let e=new g(1,500);e.addFilter(g.equals("customFieldSet.relations.entityName","product")),e.addSorting(g.sort("name","ASC")),this.customFieldRepository.search(e,it.api).then(t=>{this.productCustomFields=t})},checkMultiSelectFieldInheritance(e){return!Array.isArray(e)||e.length<=0},checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var q=`{% block weareplanet_settings_icon %}
    <span class="mt-icon icon--weareplanet-multicolor mt-icon--multicolor" style="width: 16px; height: 16px;">
...
{% endblock %}
`;var{Component:st}=Shopware;st.register("sw-weareplanet-settings-icon",{template:q});var V=`<mt-card class="mt-card"
...
`;var{Component:ct,Mixin:dt}=Shopware;ct.register("sw-weareplanet-storefront-options",{template:V,name:"WeArePlanetStorefrontOptions",mixins:[dt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l}},methods:{checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var U=`<mt-card class="mt-card"
		 :title="$tc('weareplanet-settings.settingForm.advancedOptions.cardTitle')">
	<sw-container>
		<div v-if="actualConfigData" class="weareplanet-settings-advanced-options-fields">
			<sw-inherit-wrapper
				...
				<template #content="props">
					<sw-switch-field
							...
							bordered
							:mapInheritance="props"
							:label="$tc('weareplanet-settings.settingForm.advancedOptions.webhooksUpdateEnabled.label')"
							:helpText="$tc('weareplanet-settings.settingForm.advancedOptions.webhooksUpdateEnabled.tooltipText')"
							:disabled="props.isInherited"
							:value="props.currentValue"
							@update:value="props.updateCurrentValue">
					</sw-switch-field>
			...
			<sw-inherit-wrapper
				...
				<template #content="props">
					<sw-switch-field
							...
							bordered
							:mapInheritance="props"
							:label="$tc('weareplanet-settings.settingForm.advancedOptions.paymentsUpdateEnabled.label')"
							:helpText="$tc('weareplanet-settings.settingForm.advancedOptions.paymentsUpdateEnabled.tooltipText')"
							:disabled="props.isInherited"
							:value="props.currentValue"
							@update:value="props.updateCurrentValue">
					</sw-switch-field>
...
`;var{Component:pt,Mixin:mt}=Shopware;pt.register("sw-weareplanet-advanced-options",{template:U,name:"WeArePlanetAdvancedOptions",inject:["acl"],mixins:[mt.getByName("notification")],props:{actualConfigData:{type:Object,required:!0},allConfigs:{type:Object,required:!0},selectedSalesChannelId:{required:!0},isLoading:{type:Boolean,required:!0}},data(){return{...l}},methods:{checkTextFieldInheritance(e){return typeof e!="string"?!0:e.length<=0},checkNumberFieldInheritance(e){return typeof e!="number"?!0:e.length<=0},checkBoolFieldInheritance(e){return typeof e!="boolean"}}});var H={"sw-privileges":{permissions:{parents:{weareplanet:"WeArePlanet plugin"},weareplanet:{label:"WeArePlanet berechtigungen"}}},"weareplanet-settings":{general:{descriptionTextModule:"WeArePlanet-Einstellungen",mainMenuItemGeneral:"WeArePlanet"},header:"WeArePlanet",messageNotBlank:"Dieser Wert sollte nicht leer sein.",salesChannelCard:{button:{description:"Klicken Sie auf diese Schaltfl\xE4che, um WeArePlanet als Standard-Zahlungsabwickler im ausgew\xE4hlten Vertriebskanal festzulegen",label:"WeArePlanet als Standard-Zahlungsabwickler festlegen"},messageDefaultPaymentError:"WeArePlanet als Standard-Zahlungsabwickler konnte nicht festgelegt werden..",messageDefaultPaymentUpdated:"WeArePlanet als Standard-Zahlungsabwickler wurde festgelegt."},settingForm:{credentials:{applicationKey:{label:"Application Key",tooltipText:"Der Anwendungsschl\xFCssel wird verwendet, um dieses Plugin mit der API WeArePlanet zu authentifizieren."},cardTitle:"Anmeldedaten",spaceId:{label:"Space ID",tooltipText:"Die Space ID wird verwendet, um dieses Plugin mit der API WeArePlanet zu authentifizieren."},userId:{label:"User ID",tooltipText:"Die Benutzer-ID wird verwendet, um dieses Plugin mit der WeArePlanet-API zu authentifizieren."},button:{description:"Klicken Sie auf diese Schaltfl\xE4che, um die WeArePlanet API zu testen",label:"API Verbindung testen"},alert:{title:"API-Test",successMessage:"Die Verbindung wurde erfolgreich getestet.",errorMessage:"Die Verbindung ist fehlgeschlagen. Versuchen Sie es erneut."}},messageSaveSuccess:"WeArePlanet-Einstellungen wurden gespeichert.",messageOrderDeliveryStateError:"WeArePlanet OrderDeliveryState konnte nicht gespeichert werden.",messageOrderDeliveryStateUpdated:"WeArePlanet OrderDeliveryState wurde aktualisiert.",messagePaymentMethodConfigurationError:"WeArePlanet PaymentMethodConfiguration konnte nicht gespeichert werden. Bitte \xFCberpr\xFCfen Sie Ihre Anmeldedaten.",messagePaymentMethodConfigurationUpdated:"WeArePlanet PaymentMethodConfiguration wurde registriert.",messageWebHookError:"WeArePlanet WebHook konnte nicht gespeichert werden. Bitte \xFCberpr\xFCfen Sie Ihre Zugangsdaten.",messageWebHookUpdated:"WeArePlanet WebHook wurde aktualisiert.",options:{cardTitle:"Optionen",emailEnabled:{label:"Auftragsbest\xE4tigung per E-Mail senden",tooltipText:"Wenn diese Einstellung aktiviert ist, erhalten Ihre Kunden eine E-Mail von Ihrem Gesch\xE4ft, wenn die Zahlung ihrer Bestellung autorisiert ist."},orderCloseEnabled:{label:"Bestellung bei fehlgeschlagener Zahlung offen halten",tooltipText:"Wenn diese Einstellung aktiviert ist, bleibt die Bestellung auch bei fehlgeschlagenen Zahlungen offen."},integration:{label:"Integration",options:{iframe:"Iframe",payment_page:"Payment Page"},tooltipText:"Integration"},lineItemConsistencyEnabled:{label:"Konsistenz der Einzelposten",tooltipText:"Wenn diese Option aktiviert ist, stimmen die Summen der Einzelposten in WeArePlanetPayment immer mit der Shopware-Bestellsumme \xFCberein."},productCustomFieldsAllowList:{label:"Produkt-Zusatzfelder (Allow-List)",tooltipText:"W\xE4hlen Sie die Produkt-Zusatzfelder aus, die als Positions-Attribute an WeArePlanetPayment \xFCbertragen werden. Keine Auswahl = es werden keine Zusatzfelder \xFCbertragen."},spaceViewId:{label:"Space View ID",tooltipText:"Space View ID"}},save:"Speichern",storefrontOptions:{cardTitle:"Storefront-Optionen",invoiceDownloadEnabled:{label:"Rechnung Download",tooltipText:"Wenn diese Einstellung aktiviert ist, k\xF6nnen Ihre Kunden Auftragsrechnungen von WeArePlanet herunterladen."}},advancedOptions:{cardTitle:"Erweiterte-Optionen",webhooksUpdateEnabled:{label:"Webhooks-Update",tooltipText:"Wenn diese Einstellung aktiviert ist, wird das Webhook-Update ausgel\xF6st, wenn Sie die Einstellungen speichern"},paymentsUpdateEnabled:{label:"Payments-Update",tooltipText:"Wenn diese Einstellung aktiviert ist, wird die Aktualisierung der Zahlungsmethoden ausgel\xF6st, wenn Sie die Einstellungen speichern"}},titleError:"Fehler",titleSuccess:"Erfolg"}}};var K={"sw-privileges":{permissions:{parents:{weareplanet:"WeArePlanet plugin"},weareplanet:{label:"WeArePlanet permissions"}}},"weareplanet-settings":{general:{descriptionTextModule:"WeArePlanet settings",mainMenuItemGeneral:"WeArePlanet"},header:"WeArePlanet",messageNotBlank:"This value should not be blank.",salesChannelCard:{button:{description:"Click this button to set WeArePlanet as default payment handler in the selected SalesChannel",label:"Set WeArePlanet as default payment handler"},messageDefaultPaymentError:"WeArePlanet as default payment could not be set.",messageDefaultPaymentUpdated:"WeArePlanet as default payment has been set."},settingForm:{credentials:{applicationKey:{label:"Application Key",tooltipText:"The Application Key is used to authenticate this plugin with the WeArePlanet API."},cardTitle:"Credentials",spaceId:{label:"Space ID",tooltipText:"The space ID is used to authenticate this plugin with the WeArePlanet API."},userId:{label:"User ID",tooltipText:"The user ID is used to authenticate this plugin with the WeArePlanet API."},button:{description:"Click this button to test the WeArePlanet API",label:"API connection test"},alert:{title:"API Test",successMessage:"The connection was successfully tested.",errorMessage:"The connection was failed. Try it again."}},messageSaveSuccess:"WeArePlanet settings have been saved.",messageOrderDeliveryStateError:"WeArePlanet OrderDeliveryState could not be saved.",messageOrderDeliveryStateUpdated:"WeArePlanet OrderDeliveryState has been updated.",messagePaymentMethodConfigurationError:"WeArePlanet PaymentMethodConfiguration could not be saved. Please check your credentials.",messagePaymentMethodConfigurationUpdated:"WeArePlanet PaymentMethodConfiguration has been registered.",messageWebHookError:"WeArePlanet WebHook could not be saved. Please check your credentials.",messageWebHookUpdated:"WeArePlanet WebHook has been updated.",messageHeadlessIntegrationError:"Iframe integration is only supported for Storefront Sales Channels.",messageGlobalIframeError:"Iframe integration cannot be set globally because you have non-Storefront Sales Channels.",options:{cardTitle:"Options",emailEnabled:{label:"Send order confirmation email",tooltipText:"If this setting is enabled your customers will receive an email from your store when their order payment is authorised"},orderCloseEnabled:{label:"Keep order open on failed payment",tooltipText:"If this setting is enabled the order will be kept open for failed payments"},integration:{label:"Integration",options:{iframe:"Iframe",payment_page:"Payment Page"},tooltipText:"Integration"},lineItemConsistencyEnabled:{label:"Line item consistency",tooltipText:"If this option is enabled line item totals in WeArePlanetPayment will always match Shopware order total"},productCustomFieldsAllowList:{label:"Product custom fields (allow list)",tooltipText:"Select the product custom fields that are transmitted as line item attributes to WeArePlanetPayment. No selection = no custom fields are transmitted."},spaceViewId:{label:"Space View ID",tooltipText:"Space View ID"}},save:"Save",storefrontOptions:{cardTitle:"Storefront Options",invoiceDownloadEnabled:{label:"Invoice Download",tooltipText:"If this setting is enabled your customers will be able to download order invoices from WeArePlanet"}},advancedOptions:{cardTitle:"Advanced Options",webhooksUpdateEnabled:{label:"Webhooks Update",tooltipText:"If this setting is enabled webhook update will be triggered when you save settings"},paymentsUpdateEnabled:{label:"Payments Update",tooltipText:"If this setting is enabled payment methods update will be triggered when you save settings"}},titleError:"Error",titleSuccess:"Success"}}};var Q={"sw-privileges":{permissions:{parents:{weareplanet:"WeArePlanet brancher"},weareplanet:{label:"WeArePlanet autorisations"}}},"weareplanet-settings":{general:{descriptionTextModule:"Param\xE8tres de WeArePlanet",mainMenuItemGeneral:"WeArePlanet"},header:"WeArePlanet",messageNotBlank:"Cette valeur ne doit pas \xEAtre vide.",salesChannelCard:{button:{description:"Cliquez sur ce bouton pour d\xE9finir WeArePlanet comme gestionnaire de paiement par d\xE9faut dans le canal de vente s\xE9lectionn\xE9.",label:"D\xE9finir WeArePlanet comme gestionnaire de paiement par d\xE9faut"},messageDefaultPaymentError:"WeArePlanet comme paiement par d\xE9faut n'a pas pu \xEAtre d\xE9fini.",messageDefaultPaymentUpdated:"WeArePlanet comme paiement par d\xE9faut a \xE9t\xE9 d\xE9fini."},settingForm:{credentials:{applicationKey:{label:"Application Key",tooltipText:"La cl\xE9 d'application est utilis\xE9e pour authentifier ce plugin avec l'API."},cardTitle:"R\xE9f\xE9rences",spaceId:{label:"Space ID",tooltipText:"L'ID de l'espace est utilis\xE9 pour authentifier ce plugin avec l'API WeArePlanet.."},userId:{label:"User ID",tooltipText:"L'ID utilisateur est utilis\xE9 pour authentifier ce plugin avec l'API WeArePlanet."},button:{description:"Cliquez sur ce bouton pour tester l'API WeArePlanet.",label:"Test de connexion \xE0 l'API"},alert:{title:"Test API",successMessage:"La connexion a \xE9t\xE9 test\xE9e avec succ\xE8s.",errorMessage:"La connexion a \xE9chou\xE9. R\xE9essayez."}},messageSaveSuccess:"Les param\xE8tres de WeArePlanet ont \xE9t\xE9 enregistr\xE9s.",messageOrderDeliveryStateError:"Les param\xE8tres de WeArePlanet OrderDeliveryState n'ont pas pu \xEAtre enregistr\xE9s.",messageOrderDeliveryStateUpdated:"WeArePlanet OrderDeliveryState a \xE9t\xE9 mis \xE0 jour.",messagePaymentMethodConfigurationError:"WeArePlanet PaymentMethodConfiguration n'a pas pu \xEAtre enregistr\xE9. Veuillez v\xE9rifier vos informations d'identification.",messagePaymentMethodConfigurationUpdated:"WeArePlanet PaymentMethodConfiguration a \xE9t\xE9 enregistr\xE9.",messageWebHookError:"WeArePlanet WebHook n'a pas pu \xEAtre enregistr\xE9. Veuillez v\xE9rifier vos informations d'identification.",messageWebHookUpdated:"WeArePlanet WebHook a \xE9t\xE9 mis \xE0 jour.",options:{cardTitle:"Options",emailEnabled:{label:"Envoyer un e-mail de confirmation de commande",tooltipText:"Si ce param\xE8tre est activ\xE9, vos clients recevront un e-mail de votre magasin lorsque le paiement de leur commande sera autoris\xE9"},orderCloseEnabled:{label:"Garder la commande ouverte en cas de paiement \xE9chou\xE9",tooltipText:"Si ce param\xE8tre est activ\xE9, la commande sera gard\xE9e ouverte en cas d'\xE9chec de paiement."},integration:{label:"Integration",options:{iframe:"Iframe",payment_page:"Page de paiement"},tooltipText:"Integration"},lineItemConsistencyEnabled:{label:"Coh\xE9rence des postes de ligne",tooltipText:"Si cette option est activ\xE9e, les totaux des articles dans WeArePlanetPayment correspondront toujours au total de la commande Shopware."},productCustomFieldsAllowList:{label:"Champs personnalis\xE9s produit (liste blanche)",tooltipText:"S\xE9lectionnez les champs personnalis\xE9s produit qui sont transmis comme attributs de ligne \xE0 WeArePlanetPayment. Aucune s\xE9lection = aucun champ personnalis\xE9 n'est transmis."},spaceViewId:{label:"Space View ID",tooltipText:"Space View ID"}},save:"Enregistrer",storefrontOptions:{cardTitle:"Storefront Options",invoiceDownloadEnabled:{label:"T\xE9l\xE9chargement de facture",tooltipText:"Si ce param\xE8tre est activ\xE9, vos clients pourront t\xE9l\xE9charger les factures de commande depuis WeArePlanet"}},advancedOptions:{cardTitle:"Options avanc\xE9es",webhooksUpdateEnabled:{label:"Mise \xE0 jour des webhooks",tooltipText:"Si ce param\xE8tre est activ\xE9, la mise \xE0 jour des webhooks sera d\xE9clench\xE9e lorsque vous enregistrerez les param\xE8tres."},paymentsUpdateEnabled:{label:"Mise \xE0 jour des paiements",tooltipText:"Si ce param\xE8tre est activ\xE9, la mise \xE0 jour des m\xE9thodes de paiement sera d\xE9clench\xE9e lorsque vous enregistrez les param\xE8tres."}},titleError:"Erreur",titleSuccess:"Succ\xE8s"}}};var Y={"sw-privileges":{permissions:{parents:{weareplanet:"WeArePlanet brancher"},weareplanet:{label:"WeArePlanet autorisations"}}},"weareplanet-settings":{general:{descriptionTextModule:"Impostazioni WeArePlanet",mainMenuItemGeneral:"WeArePlanet"},header:"WeArePlanet",messageNotBlank:"Questo valore non dovrebbe essere vuoto.",salesChannelCard:{button:{description:"Fai clic su questo pulsante per impostare WeArePlanet come gestore di pagamento predefinito nel SalesChannel selezionato",label:"Imposta WeArePlanet come gestore di pagamento predefinito"},messageDefaultPaymentError:"Non \xE8 stato possibile impostare WeArePlanet come pagamento predefinito.",messageDefaultPaymentUpdated:"WeArePlanet come pagamento predefinito \xE8 stato impostato."},settingForm:{credentials:{applicationKey:{label:"Chiave di applicazione",tooltipText:"La chiave dell'applicazione \xE8 usata per autenticare questo plugin con l'API WeArePlanet."},cardTitle:"Credenziali",spaceId:{label:"ID spazio",tooltipText:"L'ID dello spazio \xE8 usato per autenticare questo plugin con l'API WeArePlanet."},userId:{label:"ID utente",tooltipText:"L'ID utente \xE8 usato per autenticare questo plugin con l'API WeArePlanet."},button:{description:"Fare clic su questo pulsante per testare l'API WeArePlanet.",label:"Test di connessione API"},alert:{title:"Test API",successMessage:"La connessione \xE8 stata testata con successo.",errorMessage:"La connessione \xE8 fallita. Riprovare."}},messageSaveSuccess:"Le impostazioni di WeArePlanet sono state salvate.",messageOrderDeliveryStateError:"WeArePlanet OrderDeliveryState non pu\xF2 essere salvato.",messageOrderDeliveryStateUpdated:"WeArePlanet OrderDeliveryState \xE8 stato aggiornato.",messagePaymentMethodConfigurationError:"WeArePlanet PaymentMethodConfiguration non pu\xF2 essere salvato. Per favore controlla le tue credenziali.",messagePaymentMethodConfigurationUpdated:"WeArePlanet PaymentMethodConfiguration \xE8 stato registrato.",messageWebHookError:"WeArePlanet WebHook non pu\xF2 essere salvato. Per favore controlla le tue credenziali.",messageWebHookUpdated:"WeArePlanet WebHook \xE8 stato aggiornato.",options:{cardTitle:"Opzioni",emailEnabled:{label:"Invia email di conferma dell'ordine",tooltipText:"Se questa impostazione \xE8 abilitata i tuoi clienti riceveranno un'email dal tuo negozio quando il pagamento del loro ordine sar\xE0 autorizzato"},orderCloseEnabled:{label:"Mantieni l'ordine aperto in caso di pagamento non riuscito.",tooltipText:"Se questa impostazione \xE8 abilitata, l'ordine rimarr\xE0 aperto in caso di mancato pagamento."},integration:{label:"Integrazione",options:{iframe:"Iframe",payment_page:"Pagina di pagamento"},tooltipText:"Integrazione"},lineItemConsistencyEnabled:{label:"Coerenza dell'elemento linea",tooltipText:"Se questa opzione \xE8 abilitata i totali degli articoli in WeArePlanetPayment corrisponderanno sempre al totale dell'ordine Shopware"},productCustomFieldsAllowList:{label:"Campi personalizzati prodotto (allow list)",tooltipText:"Selezionare i campi personalizzati del prodotto che vengono trasmessi come attributi di posizione a WeArePlanetPayment. Nessuna selezione = nessun campo personalizzato viene trasmesso."},spaceViewId:{label:"ID della vista spazio",tooltipText:"ID della vista spaziale"}},save:"Salva",storefrontOptions:{cardTitle:"Opzioni vetrina",invoiceDownloadEnabled:{label:"Scaricamento fattura",tooltipText:"Se questa impostazione \xE8 abilitata i tuoi clienti potranno scaricare le fatture degli ordini da WeArePlanet"}},advancedOptions:{cardTitle:"Opzioni avanzate",webhooksUpdateEnabled:{label:"Aggiornamento webhooks",tooltipText:"Se questa impostazione \xE8 abilitata l'aggiornamento dei webhook sar\xE0 attivato quando si salvano le impostazioni"},paymentsUpdateEnabled:{label:"Aggiornamento pagamenti",tooltipText:"Se questa impostazione \xE8 abilitata l'aggiornamento dei metodi di pagamento verr\xE0 attivato quando si salvano le impostazioni"}},titleError:"Errore",titleSuccess:"Successo"}}};var{Module:_t}=Shopware;_t.register("weareplanet-settings",{type:"plugin",name:"WeArePlanet",title:"weareplanet-settings.general.descriptionTextModule",description:"weareplanet-settings.general.descriptionTextModule",color:"#28d8ff",icon:"default-action-settings",version:"1.0.1",targetVersion:"1.0.1",snippets:{"de-DE":H,"en-GB":K,"fr-FR":Q,"it-IT":Y},routes:{index:{component:"weareplanet-settings",path:"index",meta:{parentPath:"sw.settings.index",privilege:"weareplanet.viewer"},props:{default:e=>({hash:e.params.hash})}}},settingsItem:{group:"plugins",to:"weareplanet.settings.index",iconComponent:"sw-weareplanet-settings-icon",backgroundEnabled:!0,privilege:"weareplanet.viewer"}});var m=Shopware.Classes.ApiService,f=class extends m{constructor(t,a,n="weareplanet"){super(t,a,n)}registerWebHooks(t=null){let a=this.getBasicHeaders(),n=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/configuration/register-web-hooks`;return this.httpClient.post(n,{salesChannelId:t},{headers:a}).then(r=>m.handleResponse(r))}checkApiConnection(t=null,a=null,n=null){let r=this.getBasicHeaders(),s=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/configuration/check-api-connection`;return this.httpClient.post(s,{spaceId:t,userId:a,applicationId:n},{headers:r}).then(o=>m.handleResponse(o))}setWeArePlanetAsSalesChannelPaymentDefault(t=null){let a=this.getBasicHeaders(),n=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/configuration/set-weareplanet-as-sales-channel-payment-default`;return this.httpClient.post(n,{salesChannelId:t},{headers:a}).then(r=>m.handleResponse(r))}synchronizePaymentMethodConfiguration(t=null){let a=this.getBasicHeaders(),n=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/configuration/synchronize-payment-method-configuration`;return this.httpClient.post(n,{salesChannelId:t},{headers:a}).then(r=>m.handleResponse(r))}installOrderDeliveryStates(){let t=this.getBasicHeaders(),a=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/configuration/install-order-delivery-states`;return this.httpClient.post(a,{},{headers:t}).then(n=>m.handleResponse(n))}},Z=f;var h=Shopware.Classes.ApiService,b=class extends h{constructor(t,a,n="weareplanet"){super(t,a,n)}createRefund(t,a,n,r){let s=this.getBasicHeaders(),o=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/refund/create-refund/`;return this.httpClient.post(o,{salesChannelId:t,transactionId:a,quantity:n,lineItemId:r},{headers:s}).then(i=>h.handleResponse(i))}createRefundByAmount(t,a,n){let r=this.getBasicHeaders(),s=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/refund/create-refund-by-amount/`;return this.httpClient.post(s,{salesChannelId:t,transactionId:a,refundableAmount:n},{headers:r}).then(o=>h.handleResponse(o))}createPartialRefund(t,a,n,r){let s=this.getBasicHeaders(),o=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/refund/create-partial-refund/`;return this.httpClient.post(o,{salesChannelId:t,transactionId:a,refundableAmount:n,lineItemId:r},{headers:s}).then(i=>h.handleResponse(i))}},j=b;var J=Shopware.Classes.ApiService,_=class extends J{constructor(t,a,n="weareplanet"){super(t,a,n)}getTransactionData(t,a){let n=this.getBasicHeaders(),r=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/transaction/get-transaction-data/`;return this.httpClient.post(r,{salesChannelId:t,transactionId:a},{headers:n}).then(s=>J.handleResponse(s))}getInvoiceDocument(t,a){return`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/transaction/get-invoice-document/${t}/${a}`}getPackingSlip(t,a){return`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/transaction/get-packing-slip/${t}/${a}`}},X=_;var ee=Shopware.Classes.ApiService,w=class extends ee{constructor(t,a,n="weareplanet"){super(t,a,n)}createTransactionCompletion(t,a){let n=this.getBasicHeaders(),r=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/transaction-completion/create-transaction-completion/`;return this.httpClient.post(r,{salesChannelId:t,transactionId:a},{headers:n}).then(s=>ee.handleResponse(s))}},te=w;var ae=Shopware.Classes.ApiService,I=class extends ae{constructor(t,a,n="weareplanet"){super(t,a,n)}createTransactionVoid(t,a){let n=this.getBasicHeaders(),r=`${Shopware.Context.api.apiPath}/_action/${this.getApiBasePath()}/transaction-void/create-transaction-void/`;return this.httpClient.post(r,{salesChannelId:t,transactionId:a},{headers:n}).then(s=>ae.handleResponse(s))}},ne=I;var{Application:u}=Shopware;u.addServiceProvider("WeArePlanetConfigurationService",e=>{let t=u.getContainer("init");return new Z(t.httpClient,e.loginService)});u.addServiceProvider("WeArePlanetRefundService",e=>{let t=u.getContainer("init");return new j(t.httpClient,e.loginService)});u.addServiceProvider("WeArePlanetTransactionService",e=>{let t=u.getContainer("init");return new X(t.httpClient,e.loginService)});u.addServiceProvider("WeArePlanetTransactionCompletionService",e=>{let t=u.getContainer("init");return new te(t.httpClient,e.loginService)});u.addServiceProvider("WeArePlanetTransactionVoidService",e=>{let t=u.getContainer("init");return new ne(t.httpClient,e.loginService)});})();
